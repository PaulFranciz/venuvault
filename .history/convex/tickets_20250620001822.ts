...
export const getUserTicketForEventOptimized = query({
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    try {
      // Parallel fetch ticket and event data
      ...
      if (!ticket) {
        console.log(`[OPTIMIZED] getUserTicketForEventOptimized: No ticket found for user ${userId}, event ${eventId} (${duration}ms)`);
        return null;
      }

      console.log(`[OPTIMIZED] getUserTicketForEventOptimized: Found ticket in ${duration}ms`);
...
// Keep original functions for backward compatibility but mark as deprecated
export const getUserTicketForEvent = query({
  args: {
    eventId: v.id("events"),
    userId: v.string(),
  ...
  handler: async (ctx, { eventId, userId }) => {
    ...
    return ctx.runQuery(internal.tickets.getUserTicketForEventOptimized, { eventId, userId });
  },
});
