...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    const { 
      ...
      locationTips, coordinates, placeId, endDate, startTime, endTime, timezone, category,
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo }) => {
    ...
    try {
      ...
      console.log("Updating event attendeeCount");
      const currentAttendeeCount = event.attendeeCount || 0;
      await ctx.db.patch(eventId, {
        attendeeCount: currentAttendeeCount + quantity,
      });
...
// PHASE 2: High-performance getUserTickets with optimized parallel execution
export const getUserTickets = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    try {
      ...
      // Step 4: Filter tickets with relaxed criteria to support both paid and free tickets
      const validTickets = tickets.filter(ticket => {
        // Only exclude obvious test tickets with very specific patterns
        if (ticket.paystackReference && 
            (ticket.paystackReference === 'TEST_ONLY_REFERENCE' ||
             ticket.paystackReference.startsWith('INTERNAL_TEST_') ||
             ticket.paystackReference === 'DEBUG_TEST_TICKET')) {
          console.log(`[FILTER] Excluding test ticket: ${ticket.paystackReference}`);
          return false;
        }
        
        // Include tickets with valid status regardless of payment method
        // This supports both paid tickets (with paystackReference) and free tickets (without or with 'FREE' reference)
        const isValidStatus = ticket.status === 'valid' || ticket.status === 'used';
        
        if (!isValidStatus) {
          console.log(`[FILTER] Excluding ticket with status: ${ticket.status}`);
        }
        
        return isValidStatus;
      });

      // Step 5: Combine valid tickets with events using map lookup (fastest)
      const ticketsWithEvents = validTickets.map(ticket => ({
        ...ticket,
        event: eventMap.get(ticket.eventId) || null
      }));

      const duration = Date.now() - startTime;
      console.log(`[OPTIMIZED] getUserTickets: Processed ${tickets.length} total tickets, ${validTickets.length} valid tickets, ${eventIds.length} unique events in ${duration}ms`);
      
      // Log details for debugging
      if (validTickets.length > 0) {
        console.log(`[OPTIMIZED] Valid tickets details:`, validTickets.map(t => ({
          id: t._id,
          status: t.status,
          paystackRef: t.paystackReference || 'FREE/NO_REF',
          eventId: t.eventId
        })));
      }
  ...
  handler: async (ctx, { userId }) => {
    ...
    const eventsWithMetrics = await Promise.all(
      events.map(async (event) => {
        ...
        // Calculate revenue from actual ticket amounts, fallback to event price
        let revenue = 0;
        validTickets.forEach(ticket => {
          if (ticket.amount && ticket.amount > 0) {
            revenue += ticket.amount;
          } else {
            // Fallback to event price if ticket amount is not set
...
export const getUserEvents = query({
  args: { userId: v.string() },
  handler: async (ctx, { userId }) => {
    // Return events created by the specified user
    return await ctx.db
      .query("events")
...
// Migration function to fix attendeeCount for existing events
export const fixAttendeeCount = mutation({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    for (const event of events) {
      ...
      if (currentAttendeeCount !== validTicketCount) {
        await ctx.db.patch(event._id, {
          attendeeCount: validTicketCount,
        });

        results.push({
          eventId: event._id,
          eventName: event.name,
          oldCount: currentAttendeeCount,
          newCount: validTicketCount,
          fixed: true,
        });
      } else {
        results.push({
...
// Get waiting list entries for an event (for event creators to see reservations)
export const getEventWaitingList = query({
  args: { eventId: v.id("events") },
  handler: async (ctx, { eventId }) => {
    // Get all waiting list entries for this event
    const entries = await ctx.db
      .query("waitingList")
    ...
    const entriesWithUsers = await Promise.all(
      entries.map(async (entry) => {
        // Get user info from Clerk (this is a simplified approach)
        // In production, you might want to cache user info in your database
        return {
          ...entry,
          // We'll need to get user info from Clerk or store it in the database
          userInfo: {
            id: entry.userId,
    ...
    // Group by status for easier dashboard display
    const grouped = {
      waiting: entriesWithUsers.filter(e => e.status === WAITING_LIST_STATUS.WAITING),
      offered: entriesWithUsers.filter(e => e.status === WAITING_LIST_STATUS.OFFERED),
      purchased: entriesWithUsers.filter(e => e.status === WAITING_LIST_STATUS.PURCHASED),
      expired: entriesWithUsers.filter(e => e.status === WAITING_LIST_STATUS.EXPIRED),
    };
    ...
    return {
      ...
      summary: {
        ...
        totalOffered: grouped.offered.length,
        totalPurchased: grouped.purchased.length,
        totalExpired: grouped.expired.length,
        totalActive: grouped.waiting.length + grouped.offered.length,
      }
    };
  },
});

// Get real-time event analytics for dashboard
export const getEventAnalytics = query({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    const validTickets = tickets.filter(t => t.status === TICKET_STATUS.VALID || t.status === TICKET_STATUS.USED);
    const refundedTickets = tickets.filter(t => t.status === TICKET_STATUS.REFUNDED);
    const cancelledTickets = tickets.filter(t => t.status === TICKET_STATUS.CANCELLED);

    // Calculate revenue
    const totalRevenue = validTickets.reduce((sum, ticket) => sum + (ticket.amount || 0), 0);
    const refundedAmount = refundedTickets.reduce((sum, ticket) => sum + (ticket.amount || 0), 0);

    // Recent activity
    const recentTicketSales = tickets.filter(t => t.purchasedAt > last24Hours);
    const recentWaitingListJoins = waitingListEntries.filter(e => e._creationTime > last24Hours);

    // Active reservations (offered status with valid expiry)
    const activeReservations = waitingListEntries.filter(e => 
      e.status === WAITING_LIST_STATUS.OFFERED && 
      (e.offerExpiresAt || 0) > now
    );

    // Conversion metrics
    const totalInterested = waitingListEntries.length; // Total people who showed interest
    const conversionRate = totalInterested > 0 ? (validTickets.length / totalInterested) * 100 : 0;
    ...
    return {
      ...
      tickets: {
        ...
        remaining: Math.max(0, event.totalTickets - validTickets.length),
      },
      revenue: {
        total: totalRevenue,
      ...
      waitingList: {
        totalWaiting: waitingListEntries.filter(e => e.status === WAITING_LIST_STATUS.WAITING).length,
        activeReservations: activeReservations.length,
        totalInterested: totalInterested,
        conversionRate: Math.round(conversionRate * 100) / 100,
      },
      recentActivity: {
        ticketsSoldLast24h: recentTicketSales.length,
        newWaitingListLast24h: recentWaitingListJoins.length,
        activeReservationsCount: activeReservations.length,
      },
      timeline: {
        // Get hourly breakdown for the last 24 hours
        hourlyData: await getHourlyTicketSales(ctx, eventId, last24Hours, now),
      }
    };
  },
});

// Helper function to get hourly ticket sales data
async function getHourlyTicketSales(ctx: any, eventId: any, startTime: number, endTime: number) {
  const tickets = await ctx.db
    .query("tickets")
    .filter((q: any) => 
      q.and(
        q.eq(q.field("eventId"), eventId),
        q.gte(q.field("purchasedAt"), startTime)
      )
    )
    .collect();
  ...
  for (let time = startTime; time < endTime; time += hourMs) {
    ...
    const ticketsInHour = tickets.filter((t: any) => 
      t.purchasedAt >= time && t.purchasedAt < hourEnd &&
    ...
    hourlyData.push({
      ...
      timestamp: time,
      ticketsSold: ticketsInHour.length,
      revenue: ticketsInHour.reduce((sum: number, t: any) => sum + (t.amount || 0), 0),
    });
  }
...
// Debug helper queries
export const getAllTicketsForUser = query({
  args: { userId: v.string() },
  handler: async (ctx, { userId }) => {
    // Get ALL tickets for user without any filtering for debugging
    const tickets = await ctx.db
      .query("tickets")
      .withIndex("by_user", (q) => q.eq("userId", userId))
      .collect();
...
export const getTicketsByPaystackReference = query({
  args: { paystackReference: v.string() },
  handler: async (ctx, { paystackReference }) => {
    // Get tickets by PayStack reference for debugging
    const tickets = await ctx.db
      .query("tickets")
...
// Simple debug helper to count all tickets
export const countAllTickets = query({
  args: {},
  handler: async (ctx) => {
...
export const createTestTicket = mutation({
  ...
  handler: async (ctx, { userId, eventId, paystackReference, amount }) => {
    // Verify the event exists
    const event = await ctx.db.get(eventId);
    if (!event) {
      throw new Error("Event not found");
    ...
    // Create a test ticket with required fields and optional fields
    ...
    if (paystackReference) {
      ticketData.paystackReference = paystackReference;
    }
    if (amount !== undefined) {
    ...
    console.log(`Created test ticket: ${ticketId} for user: ${userId}`);
    return { ticketId, eventName: event.name };
  },
});

// Simple debug query without authentication
export const debugDatabaseAccess = query({
  args: {},
  handler: async (ctx) => {
    try {
      // Test basic database access
      const eventsCount = await ctx.db.query("events").collect().then(events => events.length);
      const usersCount = await ctx.db.query("users").collect().then(users => users.length);
      const ticketsCount = await ctx.db.query("tickets").collect().then(tickets => tickets.length);
