...
export const getUserTickets = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    try {
      ...
      // Step 4: Filter out test tickets and combine with events
      const validTickets = tickets.filter(ticket => {
        // More permissive filtering: only exclude obvious test tickets
        // Allow tickets without paystack reference (free events)
        if (ticket.paystackReference) {
          // Only exclude if it's explicitly a test reference
          if (ticket.paystackReference === 'test' ||
              ticket.paystackReference.includes('test_ref_') ||
              ticket.paystackReference.includes('test_protection_check') ||
              ticket.paystackReference.includes('FREE-TEST-')) {
            return false;
          }
        }
        
        // Allow more ticket statuses: valid, used, reserved, confirmed
        // Only exclude cancelled, refunded, or invalid tickets
        return !['cancelled', 'refunded', 'invalid', 'failed'].includes(ticket.status);
      });
