  ...
  handler: async (ctx, args) => {
    ...
    return {
      ...
      codes: codes.map(code => ({
        ...code,
        estimatedEarnings: code.currentUses * (code.commissionRate || 0) * 10,
      })),
    };
  },
});
...
export const generatePromoCode = action({
  ...
  handler: async (ctx, args) => {
    ...
    while (attempts < 10) {
      ...
      const existing = await ctx.runQuery(api.socialMarketing.checkPromoCodeExists, { code });
      if (!existing) break;
...
// Helper function to check if promo code exists
export const checkPromoCodeExists = query({
  args: { code: v.string() },
...
export const trackReferral = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!promoCode || (promoCode.type !== "affiliate" && promoCode.type !== "influencer")) {
