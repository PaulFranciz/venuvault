...
export const getEmailSequences = query({
  ...
  handler: async (ctx, { eventId, createdBy }) => {
    ...
    if (eventId) {
      ...
      sequences = await ctx.db.query("emailSequences").collect();
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (args.sequenceId) {
      ...
      templates = await ctx.db
        ...
        .collect();
    } else if (args.templateType !== undefined) {
      templates = await ctx.db
        .query("emailTemplates")
        .withIndex("by_type", (q) => q.eq("templateType", args.templateType!))
        .collect();
    } else {
...
export const getEmailCampaigns = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.eventId) {
      ...
      campaigns = await ctx.db.query("emailCampaigns").collect();
    }
  ...
  handler: async (ctx, args) => {
    ...
    if (args.campaignId) {
      ...
      analytics = await ctx.db
        ...
        .withIndex("by_event", (q) => q.eq("eventId", args.eventId))
        .collect();
    } else if (args.templateId !== undefined) {
      analytics = await ctx.db
        .query("emailAnalytics")
        .withIndex("by_template", (q) => q.eq("templateId", args.templateId!))
        .collect();
    } else {
      analytics = await ctx.db.query("emailAnalytics").collect();
    }
...
export const triggerAutomatedEmails = action({
  ...
  handler: async (ctx, args) => {
    ...
    const sequences: any[] = await ctx.runQuery(api.emailAutomation.getEmailSequences, {
      eventId: args.eventId,
    });

    const relevantSequences: any[] = sequences.filter((seq: any) => 
    ...
    for (const sequence of relevantSequences) {
      ...
      const templates: any[] = await ctx.runQuery(api.emailAutomation.getEmailTemplates, {
        sequenceId: sequence._id,
      });

      for (const template of templates) {
        // Create and send campaign
        const campaignId: any = await ctx.runMutation(api.emailAutomation.createEmailCampaign, {
...
export const sendEmailCampaign = action({
  ...
  returns: v.array(v.object({
    recipientEmail: v.string(),
    status: v.string(),
    analyticsId: v.id("emailAnalytics"),
  })),
  handler: async (ctx, args) => {
    const campaign: any[] = await ctx.runQuery(api.emailAutomation.getEmailCampaigns, {});
    const targetCampaign: any = campaign.find((c: any) => c._id === args.campaignId);
    ...
    if (targetCampaign.eventId) {
      if (targetCampaign.targetAudience.type === "ticket_holders") {
        ...
        recipients = tickets.map((t: any) => t.userId); // Would need to resolve to emails
      }
    }
    ...
    for (const recipient of recipients) {
      // Record analytics
      const analyticsId: any = await ctx.runMutation(api.emailAutomation.recordEmailAnalytics, {
        campaignId: args.campaignId,
