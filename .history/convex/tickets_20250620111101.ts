...
// DEBUG QUERY: Raw user tickets without filtering (for debugging purposes)
export const getUserTicketsRaw = query({
  ...
  handler: async (ctx, { userId }) => {
    ...
    try {
      // Get ALL tickets for the user without any filtering
      const tickets = await ctx.db
        .query("tickets")
        .withIndex("by_user", (q) => q.eq("userId", userId))
        .collect();
      ...
      console.error(`[DEBUG] Error getting raw tickets for user ${userId}:`, error);
      return [];
    }
  },
});
