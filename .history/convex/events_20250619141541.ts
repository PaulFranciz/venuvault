...
export const getEventDetails = query({
  ...
  handler: async (ctx, { eventId }) => {
    ...
    const organizer = await ctx.db
      .query("users")
      .filter((q) => q.eq(q.field("userId"), event.userId))
      .first();
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    const { 
      ...
      // Destructure new media storage IDs
      audioSnippetStorageId,
      videoSnippetStorageId,
      generalImageStorageIds
    } = args;
    ...
    const eventId = await ctx.db.insert("events", {
      ...
      // Add new media storage IDs to the event document
      ...(audioSnippetStorageId !== undefined && { audioSnippetStorageId }),
      ...(videoSnippetStorageId !== undefined && { videoSnippetStorageId }),
      ...(generalImageStorageIds !== undefined && { generalImageStorageIds })
    });
    return eventId;
  },
});
