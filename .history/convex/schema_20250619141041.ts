...
export default defineSchema({
  events: defineTable({
    ...
    venue: v.optional(v.string()), // Venue name separate from full address
    organizer: v.optional(v.string()), // Organizer name for display
    ...
    // Media storage IDs for additional content
    audioSnippetStorageId: v.optional(v.id("_storage")), // Audio snippet (max 30 seconds)
    videoSnippetStorageId: v.optional(v.id("_storage")), // Video snippet (max 30 seconds)
    additionalImageStorageIds: v.optional(v.array(v.id("_storage"))), // Extra images from event creation
    additionalVideoStorageIds: v.optional(v.array(v.id("_storage"))), // Extra videos from event creation
    // Event details from DetailsStep
    ...
    guestList: v.optional(v.array(v.object({
      name: v.string(),
      email: v.string(),
      role: v.string(),
    }))),
