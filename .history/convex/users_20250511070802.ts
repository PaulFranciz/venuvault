...
export const getOrCreateUser = internalMutation(
  async (ctx) => {
    ...
    if (!userEmail) {
      throw new ConvexError("User email is not available from authentication provider. Profile cannot be created/updated.");
    }
    ...
    const newUserId = await ctx.db.insert("users", {
      ...
      // onboardingComplete: false, // Explicitly set if desired, otherwise relies on schema default (undefined for optional)
      // paystackSubaccountId: undefined, // Explicitly set if desired
      // logoStorageId: undefined, // etc.
    });
