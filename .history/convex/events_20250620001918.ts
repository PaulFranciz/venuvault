...
// PHASE 2: Redirect to optimized version in tickets.ts
export const getUserTickets = query({
  args: { userId: v.string() },
  handler: async (ctx, { userId }) => {
    // Use the optimized version from tickets.ts
    return await ctx.runQuery(api.tickets.getUserTicketsOptimized, { userId });
  },
});
