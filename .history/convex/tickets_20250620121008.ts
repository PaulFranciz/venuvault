...
// DEBUG QUERY: Get ALL user tickets without filtering (temporary for debugging)
export const getUserTicketsRaw = query({
  args: { userId: v.string() },
  handler: async (ctx, { userId }) => {
    console.log(`[DEBUG] Getting raw tickets for user: ${userId}`);
    
    // Get ALL tickets for this user without any filtering
    const tickets = await ctx.db
      .query("tickets")
      .withIndex("by_user", (q) => q.eq("userId", userId))
      .collect();

    console.log(`[DEBUG] Found ${tickets.length} raw tickets for user ${userId}`);
    
    // Log details about each ticket for debugging
    tickets.forEach((ticket, index) => {
      ...
      // Check why this ticket would be filtered out
      const reasons = [];
      
      if (!ticket.paystackReference) {
        reasons.push('No paystack reference');
      } else if (ticket.paystackReference === 'test') {
        reasons.push('Test reference (test)');
      } else if (ticket.paystackReference.includes('test_ref_')) {
        reasons.push('Test reference (test_ref_)');
      } else if (ticket.paystackReference.includes('test_protection_check')) {
        reasons.push('Test reference (test_protection_check)');
      }
      
      if (ticket.status !== 'valid' && ticket.status !== 'used') {
        reasons.push(`Status is '${ticket.status}' (not valid/used)`);
      }
      
      if (reasons.length > 0) {
        console.log(`  - Would be filtered out: ${reasons.join(', ')}`);
      } else {
        console.log(`  - Would be included in results`);
      }
    });
