...
const DetailsStep: React.FC<DetailsStepProps> = ({
  ...
  // Early safety check for formData
  ...
  const [generalImageFiles, setGeneralImageFiles] = useState<File[]>(() => {
    try {
      const files = formData.generalImageFiles;
      return Array.isArray(files) ? files.filter(f => f instanceof File) : [];
    } catch (error) {
      console.warn('Error initializing generalImageFiles:', error);
      return [];
    }
  });
  const [generalVideoFiles, setGeneralVideoFiles] = useState<File[]>(() => {
    try {
      const files = formData.generalVideoFiles;
      return Array.isArray(files) ? files.filter(f => f instanceof File) : [];
    } catch (error) {
      console.warn('Error initializing generalVideoFiles:', error);
      return [];
    }
  });
  ...
  useEffect(() => {
    ...
    setGeneralImageFiles(Array.isArray(formData.generalImageFiles) ? formData.generalImageFiles : []);
    setGeneralVideoFiles(Array.isArray(formData.generalVideoFiles) ? formData.generalVideoFiles : []);
  ...
  useEffect(() => {
    ...
    try {
      if (generalImageFiles && Array.isArray(generalImageFiles) && generalImageFiles.length > 0) {
      ...
      console.warn('Error processing image files:', error);
    }
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>, fileType: 'audioSnippetFile' | 'videoSnippetFile') => {
    const file = event.target.files?.[0];
    if (!file) return;

    const mediaType = fileType === 'audioSnippetFile' ? 'audio' : 'video';
    ...
    media.onloadedmetadata = () => {
      ...
      if (media.duration > 30) {
        toast.error(`${mediaType.charAt(0).toUpperCase() + mediaType.slice(1)} snippet cannot exceed 30 seconds.`);
      } else {
  ...
  const handleMultipleFilesChange = (event: React.ChangeEvent<HTMLInputElement>, fileType: 'generalImageFiles' | 'generalVideoFiles') => {
    ...
    if (files.length > 0) {
      if (fileType === 'generalImageFiles') {
        const currentFiles = Array.isArray(generalImageFiles) ? generalImageFiles : [];
        const updatedFiles = [...currentFiles, ...files];
        setGeneralImageFiles(updatedFiles);
        setFormData({ generalImageFiles: updatedFiles });
      } else if (fileType === 'generalVideoFiles') {
        const currentFiles = Array.isArray(generalVideoFiles) ? generalVideoFiles : [];
        const updatedFiles = [...currentFiles, ...files];
