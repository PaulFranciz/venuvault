...
export default function OrganizerProfileForm({ onSubmit, onBack, initialData }: OrganizerProfileFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      let logoIdToSubmit: Id<"_storage"> | undefined = initialData?.logoStorageId;
      let bannerIdToSubmit: Id<"_storage"> | undefined = initialData?.bannerStorageId;
      ...
      const profileDataToSubmit: OrganizerProfileSubmitData = {
        ...
        logoStorageId: logoIdToSubmit,
        bannerStorageId: bannerIdToSubmit,
        socialLinks: {
        ...
        >
          {currentPreview ? (
            <>
              ...
              <div className="absolute top-2 right-2 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  ...
                  className="p-1.5 bg-white/80 hover:bg-white rounded-full shadow-md text-slate-700 hover:text-blue-600 transition-all"
                  title="Change image"
                  disabled={disabled}
                >
                  <RefreshCw size={18} />
                ...
                <button 
                  type="button"
                  onClick={onFileClear}
                  className="p-1.5 bg-white/80 hover:bg-white rounded-full shadow-md text-slate-700 hover:text-red-600 transition-all"
                  title="Remove image"
                  disabled={disabled}
                >
                  <Trash2 size={18} />
            ...
            <div className="space-y-1 text-center cursor-pointer" onClick={() => !disabled && document.getElementById(id)?.click()}>
              ...
              <p className="text-sm font-pally text-slate-500 group-hover:text-blue-600 transition-colors">
                Click to upload
              </p>
      ...
      >
        ...
        <div className="space-y-6 p-6 md:p-8 bg-white rounded-xl shadow-lg">
          ...
          <div>
            ...
            <UiInput
              ...
              required
              disabled={isLoading}
              className="w-full px-4 py-3 font-pally rounded-md border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-slate-50 disabled:cursor-not-allowed"
            />
          </div>
        </div>
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                <UiInput
                  ...
                  disabled={isLoading}
                  className="w-full px-4 py-3 font-pally rounded-r-md border-slate-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-slate-50 disabled:cursor-not-allowed border-l-0"
                />
              </div>
            </div>
            <div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-slate-200 mt-8">
          ...
          <Button
            type="submit"
            disabled={isLoading}
            className="w-full sm:w-auto px-8 py-3 font-pally-medium text-base bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-md hover:shadow-lg focus:ring-4 focus:ring-blue-300 disabled:opacity-50 flex items-center justify-center"
          >
            {isLoading ? (
              <>
