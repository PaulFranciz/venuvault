...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    } else if (currentOnboardingStep === "loading" && userProfile === undefined) {
        // This case handles when clerk is loaded, user is present, but userProfile is still undefined (initial fetch ongoing)
        // It's covered by the top loading check, but as an explicit fallback for clarity if needed.
        // No action needed here as the top condition already sets to "loading"
    } else if (userProfile === null && currentOnboardingStep !== 'welcome') {
        // This ensures if profile becomes null (e.g. error during creation or logout/login new user)
        // and we are not already on welcome, we go to welcome.
        // This is covered by the `userProfile === null` check at the top, but added for robustness if that were missed.
        setCurrentOnboardingStep("welcome");
    }
