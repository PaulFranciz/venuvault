...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    if (userProfile === undefined && user?.id) { 
      setCurrentOnboardingStep("loading");
      return;
    }
    if (!user?.id || userProfile === null) { 
        setCurrentOnboardingStep("loading"); 
        return;
    }

    if (userProfile) {
        const profileValuesExist = userProfile.name && userProfile.name.trim() !== "";
        const paystackLinked = !!userProfile.paystackSubaccountId;

        if (userProfile.onboardingComplete && paystackLinked && profileValuesExist) {
            setCurrentOnboardingStep("dashboard");
        } else if (!profileValuesExist) {
            setCurrentOnboardingStep("welcome");
        } else if (!paystackLinked) {
            setCurrentOnboardingStep("paymentSetup");
        } else { 
            // Profile values exist, paystack is linked, but onboardingComplete is false.
            // This means they finished payment, so mark onboarding as complete and go to dashboard.
            updateUserProfileMutation({ onboardingComplete: true }).then(() => {
                setCurrentOnboardingStep("dashboard");
            }).catch(error => {
                console.error("Error marking onboarding complete automatically:", error);
                setCurrentOnboardingStep("paymentSetup"); // Fallback to payment step if update fails
            });
        }
    }
  }, [userProfile, user?.id, updateUserProfileMutation]);
  ...
  useEffect(() => {
    const fetchPaystackAccStatus = async () => {
      if (userProfile?.paystackSubaccountId) {
        ...
        try {
          const result = await getPaystackSubaccountStatus(); // This action might need user context or subaccount ID passed if not using session
  ...
  const handleProfileFormSubmit = async (data: OrganizerProfileSubmitData) => {
    if (!userProfile?._id) {
        console.error("User profile ID not found for submission.");
        // TODO: Show error to user via a state variable in the form or dashboard
        return;
    }
    try {
      await updateUserProfileMutation(data); // This will merge with existing fields due to optional args
      setCurrentOnboardingStep("paymentSetup"); 
    } catch (error) {
      console.error("Failed to update profile:", error);
      // TODO: Display error to user in the OrganizerProfileForm or here
    }
  };

  const handlePaymentSetupSuccess = async () => {
    if (!userProfile?._id) {
        console.error("User profile ID not found for completing onboarding.");
        return;
    }
    try {
      // The paystackSubaccountId is updated by the Paystack server action.
      // Convex reactivity should update userProfile. We just mark onboardingComplete.
  ...
  if (currentOnboardingStep === "loading" || (userProfile === undefined && user?.id)) {
    return (
