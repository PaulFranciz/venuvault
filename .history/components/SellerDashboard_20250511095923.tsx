...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    const fetchPaystackAccStatus = async () => {
      if (userProfile?.paystackSubaccountId) {
        ...
        try {
          const status = await getPaystackSubaccountStatus(userProfile.paystackSubaccountId);
  ...
  const handlePaymentSetupSuccess = async (subaccountCode: string) => {
    ...
    try {
      await updateUserProfileMutation({
        ...
        onboardingComplete: false, // Keep false until they see dashboard once
      });
      // Refetch user profile to get the latest paystackSubaccountId and other details
      // The main useEffect should then transition to 'dashboard'
      // No, explicitly set to dashboard to show the status correctly.
      // Then, if paystack is linked and onboarding complete is false, it will mark true.
      setCurrentOnboardingStep("dashboard");
      console.log("User profile updated with Paystack subaccount ID. Current step set to dashboard.");
    } catch (error) {
  ...
  const renderStepContent = () => {
    switch (currentOnboardingStep) {
      ...
      case "welcome":
        return <WelcomeStep onContinue={() => setCurrentOnboardingStep("profileForm")} />;
      case "profileForm":
        return (
