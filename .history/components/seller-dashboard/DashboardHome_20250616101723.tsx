...
import { Plus, CalendarDays, TrendingUp, CreditCard, ArrowRight, Edit, Ticket, Clock, Users, AlertTriangle, BarChart3, Eye } from 'lucide-react';
  ...
  const metrics = React.useMemo(() => {
    ...
    events.forEach(event => {
      ...
      if (event.metrics?.soldTickets) {
        ...
        // For recent activity, we'll approximate based on event creation time
        // In a real implementation, you'd track actual purchase timestamps
        if (event._creationTime > thirtyDaysAgo) {
          ticketsSoldLast30Days += event.metrics.soldTickets;
        }
      }

      // Add to recent activity if event was created recently or has recent sales
      if (event._creationTime > thirtyDaysAgo || (event.metrics?.soldTickets > 0)) {
        recentActivity.push({
          type: 'ticket_sale',
          eventName: event.name,
          count: event.metrics?.soldTickets || 0,
    ...
    const conversionRate = totalInterested > 0 ? (totalTicketsSold / totalInterested) * 100 : 0;
