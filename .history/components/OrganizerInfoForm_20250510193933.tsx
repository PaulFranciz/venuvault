...
import { motion } from 'framer-motion'; // You'll need to install this dependency
...
interface OrganizerInfoFormProps {
  onSuccess: (data: { organizerName: string; socials: SocialLinks; logoStorageId?: Id<"_storage">; bannerStorageId?: Id<"_storage"> }) => void;
}
...
export default function OrganizerInfoForm({ onSuccess }: OrganizerInfoFormProps) {
  ...
  // Make sure these functions exist in your Convex API
  const updateOrganizerProfile = useMutation(api.users.updateUserProfile);
  const uploadFile = useAction(api.storage.generateUploadUrl);
  ...
  const uploadFileToConvex = async (file: File): Promise<Id<"_storage"> | undefined> => {
    try {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Update user profile with properly typed storageIds
      ...
      console.log("Profile details submitted to Convex successfully.");
      onSuccess({ 
        organizerName, 
        socials: socialLinks, 
        logoStorageId, 
