...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    ...
    // At this point, userProfile is loaded and not null.
    ...
    // Condition for "welcome": Early stage, no specific organizer details filled OR name is default, AND no Paystack.
        ...
        // The "profileForm" step is transient until the profile is actually submitted.
        if (currentOnboardingStep !== "profileForm") {
             setCurrentOnboardingStep("welcome");
        }
        // If currentOnboardingStep IS "profileForm", we do nothing here, allowing user to stay on profileForm.
        // After they submit profileForm, organizerDetailsFilled will become true, and this whole condition will likely be false.
    }
    // Condition for "paymentSetup": Name is NOT default, AND organizer details ARE filled, BUT Paystack is NOT linked.
    // This is the state after successfully submitting the profile form.
    else if (!isDefaultName && organizerDetailsFilled && !paystackLinked) {
        setCurrentOnboardingStep("paymentSetup");
    }
    // Condition for auto-completion: Paystack IS linked, but onboarding flag is still false.
    // (Name and organizer details are implied to be filled if paystack is linked in a normal flow).
    else if (paystackLinked && !userProfile.onboardingComplete) {
         console.log("Profile, organizer details, and Paystack seem complete, but onboarding flag is false. Setting flag automatically.");
         updateUserProfileMutation({ onboardingComplete: true })
            ...
            .catch(error => {
                ...
                setCurrentOnboardingStep("paymentSetup"); // Fallback to payment setup on error
            });
    }
    // Fallback: if current step is loading and none of the above specific conditions were met.
    // This might happen if userProfile is in an unexpected intermediate state not caught by the more specific rules.
    else if (currentOnboardingStep === "loading") {
        console.warn("SellerDashboard: Fallback from loading state; conditions not specifically met. Defaulting to welcome.");
        setCurrentOnboardingStep("welcome");
    }

  }, [isClerkLoaded, user, userProfile, updateUserProfileMutation, currentOnboardingStep]); // Added currentOnboardingStep
