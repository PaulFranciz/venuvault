...
export function createRedisClient() {
  ...
  if (typeof window === 'undefined' && process.env.NODE_ENV === 'production') {
    ...
    const redisClient = new IORedis(redisUrl, {
      ...
      maxRetriesPerRequest: 2, // Reduced from 3 for faster failover
      retryDelayOnFailover: 100, // Quick retry on failover
      enableReadyCheck: false, // Skip ready check for faster connections
      maxLoadingTimeout: 3000, // 3 seconds max for initial connection
      ...
      // Retry Strategy (OPTIMIZED)
      retryStrategy: (times) => {
        if (times > 2) return null; // Give up after 2 retries
        return Math.min(times * 100, 1000); // Fast exponential backoff
      },
