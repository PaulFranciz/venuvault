...
export function generateMapEmbedUrl(location: { address?: string; coordinates?: { lat: number; lng: number } }): string {
  if (location.coordinates) {
    return `https://www.google.com/maps/embed/v1/view?key=${GOOGLE_MAPS_API_KEY}&center=${location.coordinates.lat},${location.coordinates.lng}&zoom=15`;
  }
  if (location.address) {
    const encodedAddress = encodeURIComponent(location.address);
    return `https://www.google.com/maps/embed/v1/place?key=${GOOGLE_MAPS_API_KEY}&q=${encodedAddress}&zoom=15`;
  }
  return ''; // Return an empty string or a default URL if no location is provided
}
