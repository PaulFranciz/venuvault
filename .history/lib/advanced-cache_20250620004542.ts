...
class AdvancedCacheManager {
  ...
  async get<T>(
    ...
    try {
      ...
      // Fallback: try to return stale data if available
      try {
        const staleData = await redisClient.get(fullKey);
        if (staleData) {
  ...
  async set<T>(key: string, value: T, config: CacheConfig): Promise<void> {
    ...
    try {
      ...
      await redisClient.set(fullKey, serialized, { ex: config.ttl });
  ...
  async invalidatePattern(pattern: string): Promise<number> {
    try {
      const keys = await redisClient.keys(`advanced:*${pattern}*`);
      if (keys.length > 0) {
        await redisClient.del(...keys);
        console.log(`[ADVANCED CACHE] Invalidated ${keys.length} keys matching ${pattern}`);
      }
      return keys.length;
    } catch (error) {
