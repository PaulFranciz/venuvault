...
import EnhancedLocationStep from '@/components/event-creation/EnhancedLocationStep';
...
export default function CreateEventPage() {
  ...
  const sendAudio = useMutation(api.files.sendAudio);
  const sendVideo = useMutation(api.files.sendVideo);
  const sendImage = useMutation(api.files.sendImage);

  const handleFileUploads = async () => {
    let audioSnippetStorageId: Id<"_storage"> | undefined = undefined;
    if (formData.audioSnippetFile) {
      const postUrl = await generateUploadUrl();
      const result = await fetch(postUrl, {
        method: "POST",
        headers: { "Content-Type": formData.audioSnippetFile.type },
        body: formData.audioSnippetFile,
      });
      const { storageId } = await result.json();
      await sendAudio({ storageId, format: "mp3" });
      audioSnippetStorageId = storageId;
    }

    let videoSnippetStorageId: Id<"_storage"> | undefined = undefined;
    if (formData.videoSnippetFile) {
      const postUrl = await generateUploadUrl();
      const result = await fetch(postUrl, {
        method: "POST",
        headers: { "Content-Type": formData.videoSnippetFile.type },
        body: formData.videoSnippetFile,
      });
      const { storageId } = await result.json();
      await sendVideo({ storageId, format: "mp4" });
      videoSnippetStorageId = storageId;
    }

    const generalImageStorageIds: Id<"_storage">[] = [];
    if (formData.generalImageFiles && formData.generalImageFiles.length > 0) {
      for (const file of formData.generalImageFiles) {
        const postUrl = await generateUploadUrl();
        const result = await fetch(postUrl, {
          method: "POST",
        ...
        const { storageId } = await result.json();
        await sendImage({ storageId, format: "jpeg" });
        generalImageStorageIds.push(storageId);
      }
    }
  ...
  const handleSchedulePublish = async (scheduleDateTime: Date) => {
    ...
    try {
      const mediaStorageIds = await handleFileUploads();
      ...
      const eventData = {
        ...
        imageStorageId: formData.bannerImageStorageId,
        thumbnailImageStorageId: formData.thumbnailImageStorageId,
        ...mediaStorageIds,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const eventData = {
        ...
        price: formData.ticketTypes && formData.ticketTypes.length > 0 ? parseFloat(formData.ticketTypes[0].price) : 0,
        totalTickets: formData.ticketTypes?.reduce((sum, tt) => sum + (tt.unlimited ? Infinity : parseInt(tt.quantity, 10) || 0), 0) || 0,
        userId: authUser?.id,
        imageStorageId: formData.bannerImageStorageId,
        thumbnailImageStorageId: formData.thumbnailImageStorageId,
        ...
        timezone: formData.timezone,
        category: formData.category,
        isPublished: true, // For immediate publish
        inviteOnly: formData.inviteOnly,
        refundPolicy: formData.refundPolicyDetails,
        organizerAbsorbsFees: formData.platformFeePaidBy === 'creator',
        isFreeEvent: formData.ticketTypes?.some(tt => tt.isFree),
      ...
      const eventId = await createEventMutation(eventData as any);
  ...
  const steps: Step[] = [
    ...
    },
    {
      ...
      component: EnhancedLocationStep,
      tips: [
