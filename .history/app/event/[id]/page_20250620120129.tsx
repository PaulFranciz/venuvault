...
export default function EventPage() {
  ...
  const { data: availability, isLoading: availabilityLoading } = useEventAvailabilityWithCache(params.id as string);
  const { data: userTicket, isLoading: userTicketLoading } = useUserTicketsWithCache(params.id as string, user?.id || null);
  const { data: queuePosition, isLoading: queuePositionLoading } = useQueuePositionWithCache(params.id as string, user?.id || null);
  ...
  useEffect(() => {
    if (
      queuePosition?.data?.status === 'offered' &&
      queuePosition?.data?.offerExpiresAt &&
      queuePosition?.data?._id &&
      event &&
      params.id
    ) {
      startReservation(
        queuePosition.data.offerExpiresAt,
        queuePosition.data._id, // reservationId
        params.id as Id<"events">, // eventId
        event.name, // eventName
        imageUrl // eventBannerUrl (already derived from event image IDs)
      );
    } else if (!queuePosition?.data || queuePosition?.data?.status !== 'offered') {
      // Only reset if we're not in an offered state
      if (showReservation && !queuePosition?.data) {
        cancelReservation();
      }
    }
  }, [
  ...
  // If we don't have an event and we're not loading, show an error
  if (!event && !eventLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#fcf9f4]">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#502413] mb-4">Event Not Found</h1>
          <p className="text-gray-600 mb-6">The event you're looking for doesn't exist or has been removed.</p>
          <button 
            onClick={() => router.push('/discover')}
            className="bg-[#F96521] hover:bg-[#e55511] text-white px-6 py-3 rounded-lg transition-all duration-200"
          >
            ...
            <div className="py-4 bg-white p-6 rounded-xl shadow-sm">
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  {/* Logic for Virtual vs. Physical Event */}
                  {event.locationType === 'virtual' ? (
                    ...
                    <>
                      {/* For physical events, show the address and the map */}
                      <div className="text-gray-700">{event.location}</div>
                      {event.coordinates && (
                        <div className="mt-4 rounded-lg overflow-hidden border border-gray-200">
                          <GoogleMapsEmbed address={event.location} />
                        </div>
                      )}
                    </>
                  )}
          ...
          <div className="hidden md:block md:w-1/3 md:mt-0 mt-8">
            <div className="sticky top-4 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
              <div className="p-6 border-b border-gray-100">
                ...
                {userTicket?.data ? (
                  <div className="p-4 bg-green-50 rounded-lg border border-green-100 mb-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        onClick={() => router.push(`/tickets/${userTicket.data._id}`)}
                ...
                ) : queuePosition?.data && queuePosition.data.status === 'offered' ? (
            ...
            userTicket?.data ? (
              ...
              <div className="flex flex-col space-y-3">
                ...
                {queuePosition?.data?._id && (
                  <ReleaseTicket
                    eventId={params.id as Id<"events">}
                    waitingListId={queuePosition.data._id}
                  />
                )}  
              </div>
            ) : (
