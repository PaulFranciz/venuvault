...
export default function CheckoutPage() {
  ...
  const calculateFeePerTicket = (price: number) => {
    if (price === 0) return 0; // No fee on free tickets
    return (price * 0.05) + 100; // 5% + ₦100
  };
  
  const feePerTicket = calculateFeePerTicket(ticketPrice);
  ...
  const total = subtotal - discountAmount + (absorbFees ? 0 : totalFees);
                  ...
                  <>
                    <div className="flex justify-between items-center text-gray-600">
                      <div className="flex items-center gap-1">
                        ...
                        <div className="relative group">
                          ...
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2 bg-gray-800 text-white text-xs rounded hidden group-hover:block">
                            5% + ₦100 per paid ticket
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-2 my-3">
                      <Checkbox 
                        id="absorbFees" 
                        checked={absorbFees}
                        onCheckedChange={(checked) => setAbsorbFees(!!checked)}
                      />
                      <Label htmlFor="absorbFees" className="text-sm">
                        Organizer absorbs platform fees
                      </Label>
                    </div>
                  </>
                )}
