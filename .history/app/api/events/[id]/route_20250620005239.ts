...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const event = await withAdvancedCache(
      `event:${id}`,
      async () => {
        // This only runs on cache miss
        console.log(`[EVENT API] Advanced cache miss for event: ${id}, querying Convex`);
      ...
      CACHE_CONFIGS.EVENT_DATA // Use EVENT_DATA config with compression and longer TTL
    );
    ...
    return NextResponse.json(event, { 
      status: 200,
      headers: {
        'X-Data-Source': 'advanced-cached',
        'X-Response-Time': `${duration}ms`,
        'Cache-Control': `public, max-age=${CACHE_CONFIGS.EVENT_DATA.ttl}`
      }
    });
  } catch (error) {
