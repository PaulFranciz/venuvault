...
import { withFastCache } from '@/lib/advanced-cache';
import { redisKeys, REDIS_TTL } from '@/lib/redis';

// Optimized connection management for better concurrent performance
let activeConnections = 0;
const MAX_CONNECTIONS = 15; // Balanced limit for stability
const CONNECTION_TIMEOUT = 5000; // Reduced to 5 seconds for faster response
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Use fast caching for better concurrent performance
    const queueData = await withFastCache(
      ...
      30 // 30 second TTL for queue positions
    );
