...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Use optimized caching with shorter TTL for real-time accuracy
    const cacheKey = redisKeys.queuePosition(eventId, userId);
    
    const queueData = await withCache(
      cacheKey,
      async () => {
        try {
      ...
      { 
        ttl: QUEUE_POSITION_CACHE_TTL,
        // Use shorter TTL for active queue positions
        dynamicTtl: (data) => {
          return data?.inQueue ? 30 : QUEUE_POSITION_CACHE_TTL; // 30 seconds for active queue members
        }
      }
    );
