  ...
  try {
    ...
    return {
      ...
      performance: {
        ...
        isMock: (metrics as any).isMock || false
      },
...
function generateAlerts(cacheStats: any, systemMetrics: any, apiMetrics: any): string[] {
  ...
  // Cache alerts
  const hitRate = cacheStats.summary?.avgHitRate || 85; // Use estimated if no data
  if (hitRate < 70) {
    alerts.push(`Low cache hit rate: ${hitRate.toFixed(2)}%`);
  }
