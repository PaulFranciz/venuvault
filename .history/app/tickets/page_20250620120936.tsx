...
export default function MyTicketsPage() {
  ...
  // DEBUG: Also get raw tickets to see what's being filtered out
    ...
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="mb-8 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
        ...
        <div className="space-y-2 text-sm">
          ...
          <p><strong>Filtered Tickets Count:</strong> {tickets?.length || 0}</p>
          <p><strong>Raw Tickets Count (All):</strong> {rawTickets?.length || 0}</p>
          <p><strong>Query Status:</strong> {tickets === undefined ? 'Loading' : 'Loaded'}</p>
          
          {rawTickets && rawTickets.length > 0 && (
            <div className="mt-4">
              <h3 className="font-semibold">Raw Tickets Found (Before Filtering):</h3>
              <div className="max-h-40 overflow-y-auto bg-white p-2 rounded border">
                {rawTickets.map((ticket, index) => (
                  <div key={ticket._id} className="mb-2 p-2 border-b last:border-b-0">
                    <p><strong>Raw Ticket #{index + 1}</strong></p>
                    <p>Status: {ticket.status}</p>
                    <p>Paystack Ref: {ticket.paystackReference || 'NONE'}</p>
                    <p>Event ID: {ticket.eventId}</p>
                    <p>Amount: {ticket.amount || 'N/A'}</p>
          ...
          {tickets && tickets.length > 0 && (
            <div className="mt-4">
              <h3 className="font-semibold">Filtered Tickets (Displayed):</h3>
              <div className="max-h-40 overflow-y-auto bg-white p-2 rounded border">
            ...
            <div className="mt-4 p-3 bg-orange-50 rounded border border-orange-200">
              ...
              {rawTickets && rawTickets.length > 0 && (
                <p className="text-red-600 font-medium mt-2">
                  ⚠️ FILTERING ISSUE: {rawTickets.length} raw tickets exist but {tickets?.length || 0} are showing after filtering!
                </p>
              )}
            </div>
          )}
      ...
      {(tickets?.length || 0) === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-600 mb-8">
            You haven't purchased any tickets yet, or your tickets are being filtered out.
          </p>
