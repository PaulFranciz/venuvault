...
// Enhanced Ticket Card Component - moved outside to avoid hook issues
...
export default function MyTicketsPage() {
  ...
  useEffect(() => {
    if (isLoaded) {
      // Give auth state time to stabilize after redirects (especially after Paystack)
      const stabilizeTimer = setTimeout(() => {
        setAuthStateStable(true);
      }, 2000); // 2 second delay to allow Clerk auth to fully rehydrate
  ...
  // Debug logging
  console.log("Tickets query result:", { tickets, user: user?.id, isSignedIn, isLoaded, authStateStable });

  // Show loading state while auth is stabilizing or data is being fetched
  if (!isLoaded || !authStateStable) {
    return (
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center py-12">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[#F96521] mx-auto mb-4"></div>
  ...
  // Only show "not authenticated" after auth state has stabilized
  if (!isSignedIn && authStateStable) {
    return (
      <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <div className="text-center py-12">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {otherTickets.map((ticket) => (
                <RefundedTicketCard key={ticket._id} ticket={ticket} />
              ))}
            </div>
          </div>
        )}
