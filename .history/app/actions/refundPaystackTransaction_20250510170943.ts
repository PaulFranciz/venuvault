...
// Define a type for the ticket object expected from the Convex query
// This should match the structure of tickets returned by `api.tickets.getValidTicketsForEvent`
// including the new optional `currency` field.
interface TicketWithCurrency {
  _id: Id<"tickets">;
  _creationTime: number;
  eventId: Id<"events">;
  userId: string;
  purchasedAt: number;
  status: "valid" | "used" | "refunded" | "cancelled"; // Match TICKET_STATUS union
  paystackReference?: string;
  amount?: number;
  currency?: string; // Added currency
  // Add any other fields that `getValidTicketsForEvent` returns and you use.
}
...
export async function refundPaystackTransaction(eventId: Id<"events">) {
  ...
  const results = await Promise.allSettled(
    ticketsToRefund.map(async (ticket: TicketWithCurrency) => { // Use the defined type
      ...
      let determinedTicketCurrency = ticket.currency; // Directly use ticket.currency
