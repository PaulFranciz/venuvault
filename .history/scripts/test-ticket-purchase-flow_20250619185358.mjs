...
// Test user IDs
const TEST_USERS = {
  BUYER: 'user_test_buyer_001',
  ORGANIZER: 'user_2vIL4uwniO0WzDdyaAQ6CdjVW4V' // This should be the event organizer
};
...
async function testEventAnalytics() {
  ...
  try {
    ...
    if (analytics) {
      ...
      console.log(`   Valid Tickets: ${analytics.validTickets || 0}`);
      console.log(`   Refunded Amount: ${analytics.refundedAmount || 0}`);
...
async function runAllTests() {
  ...
  console.log(`   - Revenue: ${results.eventAnalytics.analytics?.totalRevenue || 0}`);
