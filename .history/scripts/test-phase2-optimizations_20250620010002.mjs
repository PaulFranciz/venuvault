...
async function testAdvancedQueryOptimization() {
  ...
  try {
    ...
    // Calculate averages (exclude first iteration to remove cold start impact)
    const userTicketsWarmResults = results.userTicketsAPI.slice(1); // Skip first iteration
    const convexWarmResults = results.convexOptimized.slice(1); // Skip first iteration
    
    const avgUserTicketsAPI = userTicketsWarmResults.length > 0 
      ? userTicketsWarmResults.reduce((sum, r) => sum + r.duration, 0) / userTicketsWarmResults.length
      : results.userTicketsAPI.reduce((sum, r) => sum + r.duration, 0) / results.userTicketsAPI.length;
      
    const avgConvexOptimized = convexWarmResults.length > 0
      ? convexWarmResults.reduce((sum, r) => sum + r.duration, 0) / convexWarmResults.length  
      : results.convexOptimized.reduce((sum, r) => sum + r.duration, 0) / results.convexOptimized.length;
