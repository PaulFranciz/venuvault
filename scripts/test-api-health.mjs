...
const ENDPOINTS = [
  {
    ...
    expectedStatus: 200,
    checkResponse: (data) => {
      // Updated validation for optimized availability API
      return data.hasOwnProperty('isSoldOut') || 
             data.hasOwnProperty('isMockData') ||
             data.hasOwnProperty('totalTickets') ||
             data.hasOwnProperty('remainingTickets');
    }
  },
  {
    name: 'Queue Position',
    ...
    checkResponse: (data) => {
      ...
      // Updated validation for optimized API response structure
      return data.hasOwnProperty('position') || 
             data.hasOwnProperty('error') || 
             data.hasOwnProperty('inQueue') || // New field name
             data.hasOwnProperty('isInQueue') || // Legacy field name
             data.hasOwnProperty('timestamp') || // Another valid indicator
             data.hasOwnProperty('eventId'); // Also a valid indicator
    }
  },
  {
    name: 'Event Details',
  ...
  },
  {
    ...
    checkResponse: (data) => {
      ...
      // New structure: should be an object with tickets array for event-specific requests
      if (typeof data === 'object' && data !== null) {
        // Check for new structure with tickets array and eventId
        if (data.hasOwnProperty('tickets') && data.hasOwnProperty('eventId')) {
          // Handle case where tickets might be a stringified array
          const ticketsValue = data.tickets;
          if (typeof ticketsValue === 'string') {
            try {
              JSON.parse(ticketsValue); // Validate it's valid JSON
              return true;
            } catch {
              return false;
            }
          }
          return Array.isArray(ticketsValue);
        }
      ...
      // Fallback: check if it's directly an array (for requests without eventId)
